...
if id == "pw":
    ...
    osd_dir = os.environ["HOME"] + "/pvfs_on_osd"
...
nodes_dir = osd_dir + "/node_list.txt"
targets_dir = osd_dir + "/target_list.txt"

devname = []
fd = open(targets_dir, "r")
while True:
	line = fd.readline()
        if line == "":
            break
...
def buildfiles():
	...
	if line.find("#clients#") > -1:
		...
		for chunk in line_chunks:
			if chunk != "#clients#":
				nodes.append(chunk)
        if not (options["osdtype"] == "none" and options["dirtype"] == "pvfs") and line.find("#osdnodes#") > -1:
                line_chunks = line.split()
                for chunk in line_chunks:
                        if chunk != "#osdnodes#":
                                nodes.append(chunk)
        if line.find("#pvfsnodes#") > -1:
                line_chunks = line.split()
                for chunk in line_chunks:
                        if chunk != "#pvfsnodes#":
                                nodes.append(chunk)
    ...
    print >>fd, "    PostCreate " + options["post_create"]
    print >>fd, "    MemberAttr " + options["member_attr"]
    print >>fd, "    CollectionObject " + options["coll_object"]
    if options["osdtype"] != "none":
    ...
    j = 0
    for n in osdnodes:
	print >>fd, "    Alias", n, "osd://" + devname[j]
	j += 1
    print >>fd, "</Aliases>"
...
def start():
	...
	for n in myibosdnodes:
		os.system("rsh " + n + " "
		    + "echo " + tabfile_contents + " \> " + tabfile);
		os.system("sudo " + initiator + " " + startcmd + " "
		    + n);
	...
	# leave it on this starting machine at least
    	os.system("rsh " + n + " " + "echo " + tabfile_contents + " > " + tabfile)
...
def status():
    ...
    print "Postcr:                 ", options["post_create"]
    print "MemberAttr:             ", options["member_attr"]
    print "CollectionObject:       ", options["coll_object"]
...
while i < len(sys.argv):
	...
	options["post_create"] = "yes"
	i += 1
    elif sys.argv[i] == "-gma":
	options["member_attr"] = "yes"
	i += 1
    elif sys.argv[i] == "-cad":
        options["coll_object"] = "yes"
        i += 1
